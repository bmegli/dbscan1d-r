...
plot(1:N, log2(times.dbscan), type="p", col="red", ylim=ylim, ylab="log time", xlab="log input size")
points(1:N, log2(times.dbscan1d), type="p", col="blue")
...
epses=seq(from=0, to=full.neighborhood, length.out = 15)
N=length(epses)
minPoints=4
...
for(i in 1:N)
{
  bench=microbenchmark(dbscan1d(data, epses[i], minPoints), dbscan::dbscan(data.matrix, epses[i], minPoints), times=100, unit="ms")
